...
| 参数名 | 默认值 | 作用说明 | 建议范围 / 备注 |
|------|--------|----------|----------------|
| `HIDE_BEYOND` | `8` | 页面中最多保留的**实时对话轮次数量**。超过该数量的旧轮次会被剪枝并缓存。 | `6 ~ 12`；数值越小，DOM 越少、性能越好 |
| `BATCH_SIZE` | `8` | 向上滚动加载历史时，**每次恢复的对话轮次数量**。 | `6 ~ 12`；过大可能导致短暂滚动抖动 |
| `MAX_CACHE_PER_CONV` | `300` | 单个会话允许缓存的**历史对话轮次上限**，用于防止内存无限增长。 | 一般无需修改；超长对话可适当调大 |
| `DEBUG` | `false` | 是否在控制台输出调试日志（剪枝、恢复、状态切换等）。 | 排查问题时可设为 `true` |

> 除上述参数外，其余常量均视为内部实现细节，不建议修改。
